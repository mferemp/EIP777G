 ...
 * 2. Run: node .master-passkey-setup.js "YOUR_PASSKEY_HERE"
...
function generateMasterPasskeyHash(passkey) {
  if (!passkey) {
    console.error('❌ USAGE: node .master-passkey-setup.js "YOUR_PASSKEY"');
    process.exit(1);
  }
  ...
  console.log('2. Click hidden TARDIS button (⚫-\' or admin trigger)');
  console.log(`3. Input passkey: ${passkey}`);
  console.log('4. Admin panel unlocks → Generate user auth keys\n');
}

// Run with provided passkey or generate random one
const passkey = process.argv[2] || crypto.randomBytes(24).toString('hex');
