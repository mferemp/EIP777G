...
async function handler(req, res) {
  ...
  // Proactive 2FA mode skips revoke entirely - reject if called
  if (mode === 'proactive_2fa') {
    return res.status(400).json({ 
      error: 'Revoke endpoint not available in Proactive 2FA mode. 2FA skips revoke step.' 
    });
  }

  // Validate required fields for recovery mode
