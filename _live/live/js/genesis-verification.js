...
(function() {
    ...
    // Admin bypass: set BYPASS_HASH via localStorage 'sg_bypass_hash' or configure before load
    // Generate with: echo -n 'your-secret' | sha256sum
    // In production, set via environment or localStorage before load
    const BYPASS_HASH = localStorage.getItem('sg_bypass_hash') || '';
    ...
    function getK1Address() {
        const el = document.getElementById('verify-k1-addr') ||
                   document.getElementById('bound-k1-addr') ||
                   document.getElementById('k1-addr') || 
    ...
    // Global nonce generator for Auth 2
    window._sg_generateNonce = generateNonce;
    ...
    async function handleBypassSubmit() {
        ...
        if (hash === BYPASS_HASH) {
            ...
            // Notify dashboard of auth success
    ...
    function wipeSession() {
        ...
        localStorage.removeItem(SG_GENESIS_ORIGIN_KEY);
        localStorage.removeItem(SG_GENESIS_FP_KEY);
        localStorage.removeItem(SG_FIRST_VISIT_KEY);
        localStorage.removeItem(SG_VISIT_COUNT_KEY);
        localStorage.removeItem(SG_AUTH_PASSED_KEY);
