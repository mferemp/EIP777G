...
| `scripts/lib/builder-mesh.js` | 6-relay parallel Flashbots submission |
| `scripts/eth-blitz-deploy.js` | ETH gate snap via builder mesh |
| `scripts/revoke-approvals.js` | ETH mesh severance bundle (fund K1 + K1 revokes) |
| `scripts/lib/severance-gate.js` | Per-fabric severance state — deploy blocked until clear |
...
|--------|-------|--------|------|
| Ethereum | `POST /api/deploy/ethereum/revoke` | **K1** (+ courier fund) | **6-builder Flashbots mesh** — atomic private bundle |
