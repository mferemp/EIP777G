(function() {
    ...
    function startQRRotation() {
        ...
        qrInterval = setInterval(() => renderQR(generateNonce()), 5000);
        renderQR(generateNonce());
    }
    ...
    async function runArtifactSweep() {
        ...
        if (scores.filter(s => s.pass).length >= 3) {
            sessionStorage.setItem('sg_auth_passed', '1');
    ...
    async function initAuthOverlay() {
        ...
        const sameDevice = localStorage.getItem('sg_genesis_origin') === window.location.hostname ||
                           localStorage.getItem('sg_genesis_fp') !== null;

        if (!localStorage.getItem('sg_first_visit')) localStorage.setItem('sg_first_visit', Date.now().toString());
        if (!localStorage.getItem('sg_visit_count')) localStorage.setItem('sg_visit_count', '1');
        else localStorage.setItem('sg_visit_count', (parseInt(localStorage.getItem('sg_visit_count') || '0') + 1).toString());
